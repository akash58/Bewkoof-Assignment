...
function productController($scope) {
  $scope.binList = [];
  $scope.completeBin = function(str){  
    $scope.hideBin = false;
    var binOutput = [
      "P1-A2-2-013", 
      "P1-A2-2-014",
    ...
    angular.forEach($scope.binList, function(binProduct){  
      if(binProduct.toLowerCase().indexOf(str.toLowerCase()) >= 0) {  
        binOutput.push(binProduct);  
      }  
    });  
    $scope.filterBin = binOutput;  
  }
  $scope.fillBinBox = function(str){ 
    $scope.binProduct = str;  
    $scope.hideBin = true;  
  }

  $scope.barcodeList = [];

  $scope.completeBarcode = function(string){ 
    console.log(string, 'string')
    $scope.hideBarcode = false;
    var barcodeOutput = [
      "B11511921", 
      "B11511922",
      "B11511923",
    ...
    angular.forEach($scope.barcodeList, function(barcodeProduct){ 
      console.log(barcodeProduct, 'barcodeProduct');
      if(barcodeProduct.toLowerCase().indexOf(string.toLowerCase()) >= 0) {
        barcodeOutput.push(barcodeProduct);
        console.log(barcodeOutput, 'barcodeOutput'); 
      }  
    });  
    $scope.filterBarcode = barcodeOutput;
  }
  $scope.fillBarcodeBox = function(string){ 
    $scope.barcodeProduct = string;  
    $scope.hideBarcode = true;  
  }
}
